...
name = '_N170_ICA_Run_1_Components_Removed';

% Start the big boy loop
%*************************************************************************
for i = 1:40    
    number = int2str(i);
    file_name = strcat('3_',number,name,'.set');
    ...
    report_card{9,1}='ICA Run 2 Total Components';
    report_card{9,2}= length(1,1);
    
    report_card{10,1}='ICA Run 2 Rejected Components Indicies';
    report_card{10,2}= index_rejected;
    
    %% Save the ICA computed set before removing anything to possibley save butt later
    ...
    save_name = strcat('4_',number,'_N170_ICA_Run_2_Computed');
    ...
    save_name = strcat('5_',number,'_N170_ICA_Run_2_Components_Removed_If_Any');
    save_path = strcat('C:\Users\ryrich\Documents\Honors Thesis\Practice_Dataset\Subject_',number,'\');
    EEG = pop_saveset( EEG, 'filename',save_name,'filepath',save_path);
